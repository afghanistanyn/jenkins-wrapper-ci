...
func (i *initMenuAuthority) InitializeData(ctx context.Context) (next context.Context, err error) {
	...
	// dev_role
	menu_dev := menus[0:5]
	if err = db.Model(&authorities[2]).Association("SysBaseMenus").Replace(menu_dev); err != nil {
		return next, err
	}

	// ops_role
	if err = db.Model(&authorities[1]).Association("SysBaseMenus").Replace(menu_dev); err != nil {
		return next, err
	}
